    ...
    function getPriceDEX(sc_input_in, des_input, sc_output_in, des_output, amount_in, PriceRate,  dexType, NameToken, NamePair, cex,action, callback) {
        ...
        switch (dexType) {
            case 'kyberswap':
                 if (action === "TokentoPair") {
                    ...
                    //https://api.zeroswap.io/quote/kyberswap?fromChain=56&fromTokenAddress=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c&toTokenDecimals=18&fromTokenDecimals=18&sellAmount=1000000000000000000&slippage=1&toTokenAddress=0x55d398326f99059fF775485246999027B3197955
       ...
       $.ajax({
            url: apiUrl,
            method: ['odos','1inch','lifi'].includes(dexType) ? 'POST' : 'GET', // Cek tipe DEX
            ...
            data: ['odos','1inch','lifi'].includes(dexType) ? JSON.stringify(requestData) : requestData, // Format data berdasarkan tipe DEX
            contentType: ['odos','1inch','lifi'].includes(dexType) ? 'application/json' : undefined, // Set `contentType` hanya jika diperlukan
